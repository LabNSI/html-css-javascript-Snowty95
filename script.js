...
function resultat() {
  ...
  // On verifie si le nom a été saisie si non ( chaine vide ) on ajoute "\n- Nom" a message
  if(f.elements["nom"].value == "")  {
    message += "\n- Nom";
  }

  // On verifie si la classe a été saisie si non ( chaine vide ) on ajoute "\n- Classe" a message
  ...
  // on verifie si deux des choix on été cochés si il ont été coché on les ajoute au compteur
  for (var i=5; i < 12; i++)  {
  ...
  // on verifie si le compteur n'est pas egale a 2 si c'est non alors il a mis trop ou pas assez de spécialités
  if(cpt!=2){
    message += "\n- Deux spécialités";
  }

  // on verifie si le message a été modifié si oui on envoie le message et on arrete la fonction
  ...
  // on verifie que NSI a été coché.
